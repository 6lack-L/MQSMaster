...
class TestPortfolioManager:
    ...
    def sample_positions(self):
        ...
        return pd.DataFrame(
            {"ticker": ["AAPL", "MSFT", "GOOGL"], "quantity": [100, 50, 25]}
        )

    def test_portfolio_initialization(self, sample_positions):
        """Test PortfolioManager initializes correctly"""
        portfolio = PortfolioManager(
            cash=50000.0, total_value=150000.0, positions_df=sample_positions
        )
    ...
    def test_portfolio_with_empty_positions(self):
        ...
        portfolio = PortfolioManager(
            cash=100000.0, total_value=100000.0, positions_df=pd.DataFrame()
        )
    ...
    def test_get_asset_weight_zero_total_value(self, sample_positions):
        ...
        portfolio = PortfolioManager(
            cash=0.0, total_value=0.0, positions_df=sample_positions
        )
...
class TestStrategyContext:
    ...
    def positions_data(self):
        """Create sample positions DataFrame"""
        return pd.DataFrame({"ticker": ["AAPL", "MSFT"], "quantity": [100, 50]})
    ...
    def test_context_with_none_time(
        self,
        market_data,
        cash_data,
        positions_data,
    ...
    def test_trade_with_invalid_ticker(
        self,
        market_data,
        cash_data,
        positions_data,
        ...
        assert any(
            "INVALID_TICKER" in rec.getMessage() and "Skip trade" in rec.getMessage()
